...
public:
    int solver(int cap,vector<int> &prices,int i,bool buy,vector<vector<vector<int>>> &dp){
        ...
        if(dp[i][cap][buy]!=-1)
            return dp[i][cap][buy];
        if(buy){
            return dp[i][cap][buy]=max(-prices[i]+solver(cap,prices,i+1,0,dp),solver(cap,prices,i+1,1,dp));
        }
        else{
            return dp[i][cap][buy]=max(+prices[i]+solver(cap-1,prices,i+1,1,dp),solver(cap,prices,i+1,0,dp));
        }
    }
